...
class EmailNotificationService {
  ...
  def sendEmail(conf: SMTPConf, envelop: EmailEnvelop): IOResult[Unit] = {
    ...
    for {
      ...
      _       <- IOResult.effect(s"Error with 'from' address [${conf.email.value}] of email")(
      ...
      _       <- IOResult.effect(s"Error when setting 'bcc' address(es) in email")(
                   message.addRecipients(Message.RecipientType.BCC, envelop.bcc.map(_.value).mkString(","))
                 )
      _       <- IOResult.effect(s"Error when setting 'cc' address(es) in email")(
