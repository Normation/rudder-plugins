...
class ChangeRequestApiImpl (
  ...
  def checkUserAction(workflowNodeId: WorkflowNodeId, target: WorkflowNodeId): Box[String] = {
    if(workflowNodeId == TwoValidationStepsWorkflowServiceImpl.Validation.id) {
      ...
      } else if(target == TwoValidationStepsWorkflowServiceImpl.Deployed.id && !CurrentUser.checkRights(AuthorizationType.Deployer.Write)) {
    ...
    } else if(workflowNodeId == TwoValidationStepsWorkflowServiceImpl.Deployment.id && !CurrentUser.checkRights(AuthorizationType.Deployer.Write)) {
            ...
            checkWorkflow match {
              case Full(_) =>
                val answer =
                  for {
                    optCR <- readChangeRequest.get(crId) ?~!(s"Could not find ChangeRequest ${id}" )
                    changeRequest <- optCR.map(Full(_)).getOrElse(Failure(s"Could not accedt ChangeRequest ${id} details cause is: change request with id ${id} does not exist."))
                    currentState  <- readWorkflow.getStateOfChangeRequest(crId)  ?~!(s"Could not find actual state of ChangeRequest ${id}" )
