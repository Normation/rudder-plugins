...
init flags =
  ...
  in
    initModel ! [ getUsersConf initModel ]
...
decodeCurrentUsersConf = decode UsersConf
...
{-- Api Calls message --}
    GetUserInfo result ->
      case result of
        Ok users ->
          let
            newModel = { model | usersConf = Just users }
          in
            (newModel, Cmd.none)
        Err err  ->
...
processApiError err model =
  let
    newModel = { model | usersConf = Nothing }
  in
...
view model =
  let
    content = case model.usersConf of
                Nothing     -> text "Waiting for data from server..."
                Just config -> displayUsersConf config
  in
    div [] [
        content
