...
sealed trait ApiAuthorizationStatus
final object ApiAuthorizationStatus {
  final case object Enabled extends ApiAuthorizationStatus
  final case class Disabled(reason: String) extends ApiAuthorizationStatus
}
 ...
 */
final case class ApiAuthorizationLicenseInfo(
    licensee  : String
  , softwareId: String
  , minVersion: String
  , maxVersion: String
...
trait CheckRudderPluginApiAuthorizationsEnable {
   ...
   */
  def enabledStatus(): ApiAuthorizationStatus

  /*
   * Information about the license. Maybe none is the plugin
   * is not a limited version.
   */
  def licenseInformation(): Option[ApiAuthorizationLicenseInfo]
}
