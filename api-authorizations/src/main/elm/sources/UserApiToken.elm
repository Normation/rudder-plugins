...
tokenPresent : TokenFeatureStatus -> Token -> Bool -> List (Html Msg)
tokenPresent featureStatus token isNewToken =
    let
        ( statusIcon, statusTxt, statusClass ) =
            case ( featureStatus, token.enabled ) of
                ( Enabled, True ) ->
...
featureWarningItem : Model -> Html Msg
...
view model =
    ul [ class "menu" ]
        (errorItem model
            ...
            :: (case model.token of
                    Just token ->
                        tokenPresent model.featureStatus token model.isNewToken

                    Nothing ->
                        tokenAbsent model.featureStatus
               )
        )
