...
initSettings initValues =
    let
        model =
            Model initValues.contextPath ColorPicker.empty ColorPicker.empty defaultSettings False
    in
    ...
    case msg of
            ...
            case result of
                Ok newSettings ->
                    let
                        ...
                        bgColor =
                            Color.toCssString settings.bgColorValue

                        txtColor =
                            Color.toCssString settings.labelColorValue
                        ...
                        wideLogoEnable =
                            settings.wideLogo.enable

                        smallLogoData =
                            case settings.smallLogo.data of
                                Just d ->
                                    d
                        ...
                        smallLogoEnable =
                            settings.smallLogo.enable

                        cssObj =
                            CssObj bgColor txtColor labelTxt displayLabel

                        --wideLogoEnable wideLogoData smallLogoEnable smallLogoData
                    in
        ...
        ToggleLogo logoType ->
            let
                ...
                newLogo =
                    { logo | enable = activate }

                newSettings =
                    case logoType of
                        "small" ->
                            { settings | smallLogo = newLogo }

                        _ ->
                            { settings | wideLogo = newLogo }
        ...
        RemoveFile logoType ->
            let
                ...
                newLogo =
                    { logo | enable = False, name = Nothing, data = Nothing }
        ...
        GotFile logoType file ->
            let
                ...
                newLogo =
                    { logo | enable = True, name = Just (File.name file) }

                ( newSettings, actionMsg ) =
                    case logoType of
                        "small" ->
                            ( { settings | smallLogo = newLogo }, Task.perform GotPreviewSmallLogo (File.toUrl file) )

                        _ ->
                            ( { settings | wideLogo = newLogo }, Task.perform GotPreviewWideLogo (File.toUrl file) )

                newModel =
                    { model | settings = newSettings, hover = False }
            in
            ( newModel
            , actionMsg
            )
        ...
        GotPreviewWideLogo url ->
            let
                ...
                newLogo =
                    { logo | enable = logo.enable, data = Just url }
