...
import Html exposing (Attribute, Html, a, button, div, form, h4, i, input, label, span, text)
...
view model =
    let
        ...
        ( wideLogoTxtInfo, wideActionBtn, wideClassBtn ) =
            case wideLogo.data of
                Just d ->
                    case wideLogo.name of
                        Just n ->
                            ( " (" ++ n ++ ")", RemoveFile "wide", "remove-btn" )

                        Nothing ->
                            ( "Unknown file name", RemoveFile "wide", "remove-btn" )

                Nothing ->
                    ( "(No file chosen)", UploadFile "wide", "upload-btn" )

        ( smallLogoTxtInfo, smallActionBtn, smallClassBtn ) =
            case smallLogo.data of
                Just d ->
                    case smallLogo.name of
                        Just n ->
                            ( " (" ++ n ++ ")", RemoveFile "small", "remove-btn" )

                        Nothing ->
                            ( "Unknown file name", RemoveFile "small", "remove-btn" )

                Nothing ->
                    ( "(No file chosen)", UploadFile "small", "upload-btn" )
        ...
        viewPreview smLogo lgLogo =
            let
                ...
                lgDisabled =
                    if lgLogo.enable && wideLogoUrl /= defaultWideLogoData then
                        ""
                ...
                smDisabled =
                    if smLogo.enable && smallLogoUrl /= defaultSmallLogoData then
                        ""
...
loginPagePreview settings =
    let
        customBarClass =
            case settings.displayBarLogin of
                True ->
                    ""
...
logoUrl settings isLoginPage =
    let
        ...
        url =
            case settings.wideLogo.enable of
                True ->
                    case settings.wideLogo.data of
                        Just d ->
                            d
