...
datasourceForm model datasource origin =
    let
        ...
        ( nameTitle, attributeTitle ) =
            if isNew then
                ( "New data source", [ style "opacity" "0.4" ] )

            else if String.isEmpty datasource.name then
                ( "Unnamed", [ style "font-style" "italic", style "color" "#777" ] )
        ...
        nameError =
            if String.isEmpty datasource.name then
        ...
        idError =
            if String.isEmpty datasource.id then
                span [ class "text-danger" ]
        ...
        headerDescription =
            if String.isEmpty datasource.description then
                text ""

            else
                p [] [ text datasource.description ]

        forms =
            List.concat [ baseForm, typeForm model.ui datasource urlError ]
    in
...
postForm ui httpData =
    let
        ...
        params =
            List.concat [ [ noParamWarning ], List.indexedMap paramForm httpData.parameters, [ newParam ] ]
...
headersForm ui httpData =
    let
        ...
        headers =
            List.concat [ [ noHeadersWarning ], List.indexedMap headerForm httpData.headers, [ newHeader ] ]
    ...
    div [ class "form-group" ]
        ...
        , if opened then
            div [ id "headers", class "well" ] headers
...
jsonPathForm httpData =
    div [ class "form-group clearfix" ]
        ...
        , div [ class "input-group" ]
            ...
            , input
                [ type_ "text"
                , class "form-control has-example-help"
                , id "jsonpath"
                , value httpData.path
...
advancedOptions datasource ui httpData =
    let
        requestTimeoutMinutes =
            httpData.requestTimeout // 60

        requestTimeoutSeconds =
            modBy 60 httpData.requestTimeout

        updateTimeoutMinutes =
            datasource.updateTimeout // 60

        updateTimeoutSeconds =
            modBy 60 datasource.updateTimeout
            ...
            div [ class "well options" ]
                ...
                , div [ class "input-group group-update-frequency" ]
                    [ input
                        [ type_ "number"
                        , class "form-control "
                        , id "requesttimeout-minutes"
                        , Html.Attributes.min "0"
                        , value (String.fromInt requestTimeoutMinutes)
                    ...
                    , input
                        [ type_ "number"
                        , class "form-control "
                        , id "requesttimeout-seconds"
                        , value (String.fromInt requestTimeoutSeconds)
                        , Html.Attributes.min "0"
                ...
                , div [ class "input-group group-update-frequency" ]
                    [ input
                        [ type_ "number"
                        , class "form-control "
                        , id "updatetimeout-minutes"
                        , Html.Attributes.min "0"
                        , value (String.fromInt updateTimeoutMinutes)
                    ...
                    , input
                        [ type_ "number"
                        , class "form-control "
                        , id "updatetimeout-seconds"
                        , value (String.fromInt updateTimeoutSeconds)
                        , Html.Attributes.min "0"
                        , Html.Attributes.max "59"
                ...
                , div [ class "input-group group-update-frequency" ]
                    [ input
                        ...
                        , value (String.fromInt httpData.maxParallelRequest)
            ...
            case schedule.type_ of
                Scheduled ->
                    ...
                    let
                        ...
                        minutes =
                            modBy 60 (schedule.duration // 60)
                    in
                    div [ class "input-group group-update-frequency mt-1 mb-2" ]
                        [ input
                            [ type_ "number"
                            , class "form-control "
                            , id "updateFrequency-hours"
                            , Html.Attributes.min "0"
                            , value (String.fromInt hours)
                        ...
                        , input
                            [ type_ "number"
                            , class "form-control "
                            , id "updateFrequency-minutes"
                            , Html.Attributes.min "0"
                            , value (String.fromInt minutes)
                        ...
                        [ div [ class "input-group" ]
                            [ label [ class "input-group-text", for "neverUpdate" ]
                                [ input
                                    [ type_ "checkbox"
                                    , id "neverUpdate"
                                    , checked (schedule.type_ /= NotScheduled)
                                    , onCheck
                                        (\b ->
                                            UpdateDataSource
                                                { datasource
                            ...
                            , label [ class "form-control", for "neverUpdate" ]
                                [ text
                                    ("Update periodically - "
                                        ++ (if schedule.type_ == NotScheduled then
                                                "Not scheduled"
                    ...
                    , li [ class "rudder-form" ]
                        [ div [ class "input-group" ]
                            [ label [ class "input-group-text", for "onMissingNoChange" ]
                                [ input
                                    [ type_ "radio"
                                    , id "onMissingNoChange"
                                    , checked (data.onMissing == NoChange)
