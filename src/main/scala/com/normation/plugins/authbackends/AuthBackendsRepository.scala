...
class AuthBackendsRepository(
  ...
  def getConfigOption(): Either[Exception, JsonAuthConfiguration] = {
    ...
    def param(desc: String, key: String) = {
      try {
        ...
        val value     = confValue.valueType() match { // arg java API :/
          case ConfigValueType.STRING | ConfigValueType.BOOLEAN | ConfigValueType.NUMBER => confValue.unwrapped().toString
          case ConfigValueType.NULL                                                      => "" // treat it like a missing, which it is likely
          case ConfigValueType.LIST | ConfigValueType.OBJECT                             => confValue.render(ConfigRenderOptions.concise())
        }
    ...
    val rootAdmin = {
      val login    = param("Login for root admin", "rudder.auth.admin.login")
      val password = param("Password for the root admin", "rudder.auth.admin.password").hideValue()
